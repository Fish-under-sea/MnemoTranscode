...
async def link_temporal_chain(
    ...
    for i in range(len(memories_ordered) - 1):
        a = await _engram_id_for_memory(session, user_id, memories_ordered[i].id)
        b = await _engram_id_for_memory(session, user_id, memories_ordered[i + 1].id)
        if not a or not b:
            continue
...
async def llm_enrich_cross_links(
    ...
    *,
    ...
    if len(memories) < 2:
        return 0
    lines = []
    for idx, m in enumerate(memories[:40]):
        snippet = (m.content_text or "")[:320].replace("\n", " ")
        lines.append(f"[{idx}] {m.title}: {snippet}")
    catalog = "\n".join(lines)
    prompt = f"""以下是同一人物（关系档案中的 Ta）的多条记忆摘要，下标从 0 开始。
...
- from_index 与 to_index 必须在 0..{len(memories) - 1} 之间，且 from_index != to_index

记忆列表：
{catalog}
"""
    try:
        data = await llm.json_complete(prompt, temperature=0.15)
...
async def enrich_after_memories_created(
    ...
    if llm is not None and len(ordered) >= 2:
        llm_n = await llm_enrich_cross_links(session, user_id, ordered, llm)
